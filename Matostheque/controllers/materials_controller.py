...
def generate_qrcode(device_id):
    ...
    webpage_link = f"https://{server_host}/matostheque/public/{device_id}"
...
def on_create_material(request):
    ...
    mutable_data["available_for_loan"] = "true"
...
def get_detailed_material(pk):
    ...
    if (material.type == 'LAB_SUPPLIES'):
        ...
        detailed_materials["availability"] = material.available_for_loan
...
def get_events_detail(pk):
    ...
    """
    events =  []
    loans =  Loans.objects.filter(material=pk).filter(loan_status__in=['Booked','Borrowed','Overdue','Closed'])
    for loan in loans:
...
def is_available(pk):
    ...
    """
    material = Materials.objects.get(pk=pk)
    if not material.available_for_loan:
        return False
    loans =  Loans.objects.filter(material=pk).filter(loan_status__in=['Booked','Borrowed'])
    if loans is not None :
        nbused = 0
        for loan in loans:
            date_fin = loan.loan_date + timedelta(days= int(loan.duration) - 1)
            if loan.loan_date <= timezone.now().date() <= date_fin:
                nbused = nbused + loan.loan_quantity
    return True