...
def latest_material(request):
    materials = []
    if Materials.objects.filter(available_for_loan=True).exists():
        i = 0
        for material in Materials.objects.filter(available_for_loan=True).order_by('-created_at')[:4]:
            materials.append({})
            materials[i]["material_id"] = material.material_id
            materials[i]["available_for_loan"] = material.available_for_loan
            ...
            if material.type == 'LAB_SUPPLIES':
                ...
                materials[i]["availability"] = material.available_for_loan
            i = i + 1
...
def get_total_count(request):
    ...
    if request.method == 'GET': 
        return JsonResponse({
            ...
            'materials_per_team': list(materials_per_team), 
        })
...
def update_Consumable_Availability():
    Materials.objects.filter(
        ...
        available_for_loan=True
    ).update(available_for_loan=False)
...
def update_material_availability(request,pk):
    try:
        ...
        if material.user_id != request.user.user_id and not request.user.is_staff:
            ...
            material.available_for_loan = request.data.get('available_for_loan')
            material.save()
