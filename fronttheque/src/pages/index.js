...
const Page = () => {
  ...
  const onConfirmAccountType = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        toast.error(t('dashboard.error_generic', "An error occurred. Please try again later."), { autoClose: false });
      } else {
        const updatedUserData = await response.json();
        await auth.updateUser(updatedUserData.user_id);
        toast.success(t('dashboard.success_update', "Your account was successfully updated!"));
      ...
      toast.error(t('dashboard.error_unexpected', "An unexpected error occurred. Please try again later."), { autoClose: false });
    }

  }, [accountChoice, t, user.user_id, auth]);
      ...
      >
        <Container maxWidth="xl">
          ...
          >
                  ...
                  <Stack spacing={1} sx={{ mt: 2 }}>
                    ...
                    <Stack alignItems="center" direction="row" spacing={2}>
                      ...
                      <Typography color="text.secondary" variant="caption">
                        {t('dashboard.addedThisMonth', 'added this month')}
                      </Typography>
                    </Stack>
                    ...
                    <Stack alignItems="center" direction="row" spacing={2}>
                      ...
                      <Typography color="text.secondary" variant="caption">
                        {t('dashboard.addedThisYear', 'added this year')}
                      </Typography>
                    </Stack>
                  </Stack>
                </CardContent>
              ...
              <Card sx={{ height: '100%' }}>
                <CardContent>
                  <Stack alignItems="flex-start" direction="row" justifyContent="space-between" spacing={3}>
                    <Stack spacing={1}>
                      <Typography color="text.secondary" variant="overline">
                        {t('dashboard.totalLoansApproved', 'Total Loans Approved')}
                      </Typography>
                      <Typography variant="h3">
                  ...
                  <Stack spacing={1} sx={{ mt: 2 }}>
                    <Stack alignItems="center" direction="row" spacing={2}>
                      ...
                      <Typography color="text.secondary" variant="caption">
                        {t('dashboard.approvedThisMonth', 'approved this month')}
                      </Typography>
                    </Stack>
                    ...
                    <Stack alignItems="center" direction="row" spacing={2}>
                      ...
                      <Typography color="text.secondary" variant="caption">
                        {t('dashboard.approvedThisYear', 'approved this year')}
                      </Typography>
                    </Stack>
                    ...
                    {loanStats.last_3_months && loanStats.last_3_months.map((item) => (
                        <Stack key={item.month} direction="row" justifyContent="space-between" sx={{ width: '100%' }}>
                            ...
                            >
                              {/* {item.month} */}
                              {t(`months.${item.month.toLowerCase()}`, item.month)}
                            </Typography>
              ...
              <Card>
                <CardContent>
                  <Typography variant="h6" gutterBottom>
                    {t('dashboard.materialsInventory', 'Materials Inventory by Team')}
                  </Typography>
                  <Divider sx={{ mb: 3 }} />
                        ...
                        <Box sx={{ mb: 1 }}>
                          <Stack direction="row" justifyContent="space-between" mb={1}>
                            ...
                            <Typography variant="body2" color="text.secondary">
                              {team.count} {t('dashboard.items', 'items')}
                            </Typography>
                          </Stack>
                          <LinearProgress 
                            variant="determinate" 
            ...
            <Grid container spacing={3} lg={12} direction="column" xs={12}>
          ...
          {open &&
            ...
            >
              <DialogTitle>{t('dashboard.welcomeTitle', 'Welcome to Matostheque!')}</DialogTitle>
              <DialogContent>
                <DialogContentText>
                  {t('dashboard.welcomeMessage', 'We are happy you joined the community. Please choose the type of account you want.')}
                </DialogContentText>
                <Grid container spacing={2} sx={{ my: 2 }} justifyContent="center" alignItems="flex-start">
                  <Grid item xs={5}>
                    <Stack
                      direction="column"
                      alignItems="center"
                    ...
                    >
                      ...
                      <Typography align="center">{t('dashboard.userRole', 'User')}</Typography>
                      <Typography align="center" variant="caption">
                        {t('dashboard.userDesc', 'A simple user can view the inventory and borrow equipments.')}
                      </Typography>
                    </Stack>
                  </Grid>
                  <Grid item xs={5}>
                    ...
                    >
                      ...
                      <Typography align="center">{t('dashboard.ownerRole', 'Owner')}</Typography>
                      <Typography align="center" variant="caption">
                        {t('dashboard.ownerDesc', 'An owner provides equipments he/she is willing to lend and can also borrow other equipments.')}
                      </Typography>
                    </Stack>
                  </Grid>
                </Grid>
              ...
              <DialogActions>
                <Button onClick={onConfirmAccountType} color="error">
                  {t('dashboard.confirmBtn', 'Confirm')}
                </Button>
              </DialogActions>
            </Dialog>}
        </Container>
