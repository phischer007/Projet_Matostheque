...
export const useLoanHandlers = (props) => {
    ...
    const handleLoanAction = async (loanId, message, priority, title,type) => {
        try {
            const {genericMessage, ownerMessage , borrowerMessage} = message;
            const csrftoken = getCookie('csrftoken');
            const response = await fetch(`${config.apiUrl}/loans/${type}/${loanId}/`, {
                method: 'PUT',
                credentials:"include",
                headers: {
    ...
    const OnApproveClick = async () => {
        ...
        await handleLoanAction(loanId, message, priority, title,type);
    };
