...
export const useNewMaterialHandlers = (data) => {
  ...
  const handleCodeNacreValidation = useCallback(
    (value) => {
      if (formData.code_nacre && formData.code_nacre != "") {
        ...
        if (!regex.test(code)) {
          setCodeError(true);
          toast.error(t('newMaterial.messages.invalidCodeNacre', "Please use a valid Code NACRE format. You can use the provided link to check if needed."), { autoClose: false });
        } else {
          setCodeError(false);
          toast.dismiss();
        }
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      setIsUploading(true);
      if (codeError) {
        toast.error(t('newMaterial.messages.invalidCodeNacreShort', "Please use a valid Code NACRE format."), { autoClose: false });
        setIsUploading(false); // Make sure to stop uploading loader
        return
      }
      ...
      if (!Object.values(newErrors).some(error => error)) {
          ...
          if (formData.type === "LAB_SUPPLIES") {
            fieldsToAppend.push({ key: 'loan_duration', value: formData.loan_duration });
          }
          else if (formData.type === "CONSUMABLES") {
            fieldsToAppend.push({ key: 'expiration_date', value: formData.expiration_date });
          }
          ...
          if (!response.ok) {
            if (response.status === 413) {
              setMessage({
                status: 'error',
                value: t('newMaterial.messages.compressImages', "Please consider compressing your images or try ulteriorly.")
              });
            } else {
              const errorMessage = await response.text();
              let decodeResponse = JSON.parse(errorMessage);
            ...
            setMessage({
              status: 'success',
              value: t('newMaterial.messages.successCreate', 'Material created successfully! You will be redirected soon.')
            });
    ...
    }, [formData, router, codeError, isDurationEnabled, selectedOwner, t]);
