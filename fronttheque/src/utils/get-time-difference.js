...
export const getTimeDifference = (date, t) => {
    // Get the current date and time
    const now = new Date();
    ...
    // If the difference is less than 1 minute, return the difference in seconds
    if (differenceInSeconds < 60) {
        return t('timeDifference.secondsAgo', '{{count}} seconds ago', { count: Math.round(differenceInSeconds) });
    } 

    // If the difference is less than 1 hour, return the difference in minutes
    else if (differenceInSeconds < 3600) {
        return t('timeDifference.minutesAgo', '{{count}} minutes ago', { count: Math.round(differenceInSeconds / 60) });
    } 
    
    // If the difference is less than 1 day, return the difference in hours
    else if (differenceInSeconds < 86400) {
        return t('timeDifference.hoursAgo', '{{count}} hours ago', { count: Math.round(differenceInSeconds / 3600) });
    } 
    
    // Otherwise, return the difference in days
    else {
        return t('timeDifference.daysAgo', '{{count}} days ago', { count: Math.round(differenceInSeconds / 86400) });
    }
  };